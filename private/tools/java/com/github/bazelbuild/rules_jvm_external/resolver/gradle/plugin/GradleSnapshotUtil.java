...
final class GradleSnapshotUtil {
  ...
  /**
   * Returns the resolved, timestamped version of a Gradle component identifier when it is a
   * timestamped (Maven-unique) snapshot, or empty otherwise.
   *
   * <p>This overload is used when the caller only has a {@link ComponentIdentifier} (e.g. from an
   * artifact resolution result) rather than the full {@link ResolvedComponentResult}. The {@code
   * version} is the base version reported by the identifier (the {@code -SNAPSHOT} form).
   */
  static Optional<String> timestampedSnapshotVersion(ComponentIdentifier identifier, String version) {
    Matcher matcher = GRADLE_SNAPSHOT_WITH_TIMESTAMP.matcher(identifier.toString());
    if (!matcher.matches()) {
      return Optional.empty();
    }
