...
def _null_shasum_artifact_not_in_repo_gets_no_urls_impl(ctx):
    ...
    # A null-sha artifact not in any repo still gets a file path (so non-timestamped
    # snapshots can be fetched from m2local), but no urls (no remote repo serves it).
    asserts.equals(
        env,
        "com/google/protobuf/protoc/4.34.0/protoc-4.34.0.jar",
        protoc["file"],
    )
