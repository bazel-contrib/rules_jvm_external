...
Behind the scenes, the script is calling `./gradlew dependencies` to generate a snippet you can paste into your WORKSPACE file directly:
...
If the list looks good, either copy the snippet into your WORKSPACE file
manually or run the command again to append it into the file:

```
$ bazel run @rules_jvm_external//generators:gradle -- $(pwd) >> WORKSPACE
```

Then, verify that the Bazel BUILD targets can be correctly generated by running
